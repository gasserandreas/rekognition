...
describe('images selector test suite', () => {
  it('should return empty object for null state', () => {
    expect(selectors.imagesStateSelector({})).toEqual({});
  });

  it('should handle imagesStateSelector', () => {
    expect(selectors.imagesStateSelector(initialState)).toEqual(initialState.images);
  });

  describe('images selector: imagesListSelector test suite', () => {
    it('should handle default', () => {
      const list = Object.values(initialState.images.byId);
      expect(selectors.imagesListSelector(initialState)).toEqual(list);
    });

    it('should handle empty ids', () => {
      expect(
        selectors.imagesListSelector({
          images: {
            ...initialState.images,
            ids: [],
          },
    ...
    it('should handle non matching ids', () => {
      ...
      expect(
        selectors.imagesListSelector({
          images: {
            ...initialState.images,
            ids: [...initialState.images.ids, 'invalid-id-1', 'invalid-id-2'],
          },
        }),
      ).toEqual(list);
    });
  ...
  it('handle default', () => {
    expect(selectors.sortedImageListSelector(initialState)).toEqual(sortedListImages);
  });
  ...
  it('handle with empty dates', () => {
    ...
    expect(selectors.sortedImageListSelector(newInitialState)).toEqual([
      {
        id: '2a2bdf23-e73f-4a2a-913e-da29926a195c',
        created: '2019-02-09T08:12:11.736Z',
        name: '2a2bdf23-e73f-4a2a-913e-da29926a195c.jpeg',
  ...
  it('handle imagesByIdSelector', () => {
    expect(selectors.imagesByIdSelector(initialState)).toEqual(initialState.images.byId);
  });

  it('handle imagesListRequestSelector', () => {
    expect(selectors.imagesListRequestSelector(initialState)).toEqual(initialState.images.listImageRequest);
  });

  // add image selectors
  it('handle addImageRequestSelector', () => {
    expect(selectors.addImageRequestSelector(initialState)).toEqual(initialState.images.addImageRequest);
  });

  it('handle addImageIsLoading', () => {
    expect(selectors.addImageIsLoading(initialState)).toEqual(initialState.images.addImageRequest.loading);
  });

  // get image selectors
  it('handle getImageRequestSelector', () => {
    expect(selectors.getImageRequestSelector(initialState)).toEqual(initialState.images.getImageRequest);
  });

  it('handle getImageIsLoading', () => {
    expect(selectors.getImageIsLoading(initialState)).toEqual(initialState.images.getImageRequest.loading);
  });
});
