...
describe('faces selector test suite', () => {
  it('should return empty object for null state', () => {
    expect(selectors.facesStateSelector({})).toEqual({});
  });

  it('should return facesState state', () => {
    expect(selectors.facesStateSelector(initialState)).toEqual(initialState.faces);
  });

  it('should return facesIdsByImageId value', () => {
    expect(selectors.facesIdsByImageIdSelector(initialState)).toEqual(initialState.faces.idsByImageId);
  });

  it('should return facesById value', () => {
    expect(selectors.facesByIdSelector(initialState)).toEqual(initialState.faces.byId);
  });
  ...
  it('should return facesByImageId value', () => {
    ...
    const faces = ids.map(id => byId[id]).filter(face => face !== undefined);

    // const labels;
    expect(selectors.facesByImageId(initialState, imageId)).toEqual(faces);
  });

  it('should return empty array for facesByImageId without imageId', () => {
    expect(selectors.facesByImageId(initialState, undefined)).toEqual([]);
  });

  it('should return empty array for facesByImageId with wrong imageId', () => {
    expect(selectors.facesByImageId(initialState, 'invalid')).toEqual([]);
  });
});
